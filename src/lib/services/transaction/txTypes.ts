...
/**
 * (Optional legacy types—consider deleting once all callers are migrated)
 *
 * export type SplitInput = { categoryId: number; amount: number | string };
 * export type CreateTxnInput = { ... amount: number | string; date: Date | string; ... }
 * export type UpdateTxnInput = { ... amount?: number | string; date?: Date | string; ... }
 */
