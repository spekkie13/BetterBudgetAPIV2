...
export function makeTransactionController(svc: TransactionService) {
    return {
        async listAllByTeam(teamId: number) {
            ...
            return rows ?
                new Response({ data: rows, status: 200, message: 'fetched all rows'}) :
                new Response({ data: null, status: 404, message: 'No transactions found' });
        },

        async getTransaction(teamId: number, id: number) {
            const row = await svc.selectByIdTeam(teamId, id);
            return row ?
                new Response({ data: row, status: 200, message: 'Successfully retrieved transaction'}) :
                new Response({ data: null, status: 404, message: 'No transaction found' });
        },

        async createTransaction(teamId: number, body: any) {
            const created = await svc.insert({ ...body, teamId });
            return created ?
                new Response({ data: created, status: 201, message: 'created new transaction'}) :
                new Response({ data: null, status: 400, message: 'No transactions created' });
        },
        ...
        async updateTransaction(teamId: number, id: number, body: TransactionPatch) {
            ...
            return updated ?
                new Response({ data: updated, status: 201, message: 'created new transaction'}) :
                new Response({ data: null, status: 400, message: 'No transactions updated' });
        },

        async deleteTransaction(teamId: number, id: number) {
            await svc.deleteByIdTeam(teamId, id);
            return new Response({ data: null, status: 204, message: 'deleted transaction'});
        },

        async selectTransactionsByType(teamId: number, type: string | null) {
            const result = await svc.selectByType(teamId, type);
            return result ?
                new Response({ data: result, status: 200, message: 'Successfully retrieved transactions' }) :
                new Response({ data: null, status: 404, message: 'No transactions found' });
        }
    }
}
